...
export class Token {
    static TokenExpireTime = 60 * 60 * 3;
    ...
    static CheckToken = async (DB: Database, TokenValue: string): Promise<Result> => {
        const TokenInfo = ThrowErrorIfFailed(await DB.Select("Tokens", ["CreateTime"], {
            TokenValue,
        }))["Results"];
    ...
    static GetUsernameByToken = async (DB: Database, TokenValue: string): Promise<Result> => {
        const TokenInfo = ThrowErrorIfFailed(await DB.Select("Tokens", ["Username"], {
            TokenValue,
        }))["Results"];
        if (TokenInfo.length === 0)
            return new Result(false, "Token not found");
